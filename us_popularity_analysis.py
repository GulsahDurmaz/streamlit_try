...
# def run_us_popularity_analysis(df, biden_df):
...
#     usa_trump_grouped = usa_df.groupby(['state_code', 'state'])['tweet'].count().reset_index()
...
#     ### Row B
#     usa_df['state'] = usa_df['state'].astype(str)
#     usa_biden_df['state'] = usa_biden_df['state'].astype(str)

#     unique_states = sorted(list(set(usa_df['state'].unique()) | set(usa_biden_df['state'].unique())))
#     selected_state = st.selectbox('Select state', unique_states)

#     # Calculate percentage for selected state
#     trump_percentage, biden_percentage = calculate_percentage_state(usa_df, usa_biden_df, selected_state)

#     # Calculate us percentage
#     us_trump_percentage, us_biden_percentage = calculate_percentage(df, biden_df)

#     # Calculate state_tweet_count / us_tweet_count
#     us_percentage = calculate_total_percentage_state(usa_df, usa_biden_df, selected_state)
...
#         filtered_df = usa_df[usa_df['state'] == selected_state]
#     else:
